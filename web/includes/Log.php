...
class Log
{
    ...
    public static function add($type, $title, $message)
    {
        ...
        self::$dbs->bind(':type', filter_var($type, FILTER_UNSAFE_RAW));
        self::$dbs->bind(':title', filter_var($title, FILTER_UNSAFE_RAW));
        self::$dbs->bind(':message', filter_var($message, FILTER_UNSAFE_RAW));
        self::$dbs->bind(':function', filter_var(self::getCaller(), FILTER_UNSAFE_RAW));
        self::$dbs->bind(':query', filter_var($_SERVER['QUERY_STRING'], FILTER_UNSAFE_RAW));
    ...
    public static function getAll($start, $limit, $search)
    {
        $query = "SELECT ad.user, l.* FROM `:prefix_log` AS l
                  ...
                  LIMIT :start, :lim";
        $query = str_replace(':search', filter_var($search, FILTER_UNSAFE_RAW, FILTER_FLAG_NO_ENCODE_QUOTES), $query);
